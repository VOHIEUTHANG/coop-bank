          ...
          <div style={{ display: 'flex', gap: '15px' }}>
            <div className='bw_load_image bw_mb_2 bw_text_center'>
              <label className='bw_choose_image'>
                {!methods.watch('gift_image_1') && (
                  <input
                    accept='image/*'
                    type='file'
                    onChange={async (_) => {
                      const base64 = await getBase64(_.target.files[0]);
                      methods.setValue('gift_image_1', base64);
                    }}
                    disabled={disabled}
                  />
                )}
                {renderImage('gift_image_1')}
              </label>
            </div>

            <div className='bw_load_image bw_mb_2 bw_text_center'>
              <label className='bw_choose_image'>
                {!methods.watch('gift_image_2') && (
                  <input
                    accept='image/*'
                    type='file'
                    onChange={async (_) => {
                      const base64 = await getBase64(_.target.files[0]);
                      methods.setValue('gift_image_2', base64);
                    }}
                    disabled={disabled}
                  />
                )}
                {renderImage('gift_image_2')}
              </label>
            </div>

            <div className='bw_load_image bw_mb_2 bw_text_center'>
              <label className='bw_choose_image'>
                {!methods.watch('gift_image_3') && (
                  <input
                    accept='image/*'
                    type='file'
                    onChange={async (_) => {
                      const base64 = await getBase64(_.target.files[0]);
                      methods.setValue('gift_image_3', base64);
                    }}
                    disabled={disabled}
                  />
                )}
