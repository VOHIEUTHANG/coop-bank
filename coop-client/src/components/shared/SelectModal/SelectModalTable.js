...
export default function SelectModalTable({ handleOpenModalGift, noLoadData, contentCreate, filedList }) {
    ...
    () => [
      {
        ...
        formatter: (p) => <b className='cb_sticky cb_name_sticky'>{p?.product_code}</b>,
      },
      {
        header: 'Tên hàng hóa - vật tư',
        formatter: (p) => (
          <div className='cb_inf_pro'>
            <img
              alt=''
              src={/[/.](gif|jpg|jpeg|tiff|png)$/i.test(p?.picture_url) ? p.picture_url : 'cb_image/img_cate_1.png'}
            />
            {p?.product_name}
          </div>
        ),
      ...
      },
      {
        ...
        formatter: (p) =>
          p?.is_active ? (
            <span className='cb_label_outline cb_label_outline_success text-center'>Có</span>
          ) : (
            <span className='cb_label_outline cb_label_outline_danger text-center'>Không</span>
          ),
      },
    ],
    [noLoadData],
